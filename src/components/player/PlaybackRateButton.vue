<template>
  <button
    type="button"
    class="relative flex h-6 w-6 items-center justify-center rounded-md text-slate-500 hover:bg-slate-100 hover:text-slate-700 focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:outline-hidden"
    @click="cyclePlaybackRate()"
  >
    <component :is="playbackRates[currentRateIdx].icon" class="h-full w-full" />
  </button>
</template>
...
const player = inject('audioPlayer')
...
const currentRateIdx = ref(0)

const cyclePlaybackRate = () => {
  currentRateIdx.value = (currentRateIdx.value + 1) % playbackRates.length
  player.playbackRate(playbackRates[currentRateIdx.value].value)
}
</script>
